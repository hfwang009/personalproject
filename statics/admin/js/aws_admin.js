var AWS =
{
	...
	loading: function (type)
	{
		...
		if (type == 'show')
		{
			AWS.G.loading = layer.load(1, {
			    shade: [0.1,'#fff'] //0.1透明度的白色背景
			});
		}
		else
		{
			layer.close(AWS.G.loading);
		}
	},

	alert: function (msg,icon,callback){
		icon = (typeof(icon) == 'undefined' || icon == '')?1:icon;
		layer.alert(
			msg, 
			{icon: icon},
			function(index){
	...
	return_message: function (msg,href,time,icon){
		var data = {};
	...
	tip_message: function(msg,eve,time,icon){
	...
	ajax_request: function(url, params)
	{
	    ...
	    function _callback (result)
	    {
			...
			if(result.state){
				if(result.code == 1){
					AWS.tip_message(result.msg,null,1000,1);
				}else if(result.code == 2){
					AWS.return_message(result.msg,result.href);
				}else if(result.code == 3){
					window.location.reload();
				}
			}else{
				AWS.tip_message(result.msg);
			}
	    }
	    ...
	    function _error (error)
	    {
            ...
            {
                AWS.tip_message(_t('发生错误, 返回的信息:') + ' ' + error.responseText);
            }
	    }
