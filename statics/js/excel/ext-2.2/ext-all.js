					...
					onRender : function(H, F) {
						if (!this.el) {
							...
							if (this.items[0].items) {
								...
								if (this.vertical) {
									var O = Math.ceil(this.items.length / D), L = 0;
									for ( var E = 0, J = this.items.length; E < J; E++) {
										if (E > 0 && E % O == 0) {
											L++
										}
		...
		.extend(
				...
				Ext.util.Observable,
				{
					...
					ensureVisible : function(P, E, D) {
						...
						return F ? Ext.fly(F).getXY() : [
								L.scrollLeft + this.el.getX(),
								Ext.fly(I).getY() ]
					},
				...
				Ext.BoxComponent,
				{
					...
					updateProgress : function(B, C) {
						...
						if (this.rendered) {
							...
							if (this.textTopEl) {
								this.textTopEl.removeClass("x-hidden")
										.setWidth(A)
							}
						}
...
Ext.History = (function() {
	...
	function H(L) {
		var K = [ "<html><body><div id=\"state\">", L, "</div></body></html>" ]
				.join("");
		try {
