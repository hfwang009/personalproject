function addGridMethods(grid) {
	...
	grid.onmousemove = function(e) {
		...
		if (grid.columnResizing) {
			grid.verticalResizer.setLeft(e.clientX - 458);
		} else {
			if (grid.rowResizing) {
				grid.horizontalResizer.setTop(e.clientY - 475);
			}
		}
	};
}
...
function addGridRowEvents(grid, row) {
	row.on("resizemousedown", function(xrow, e) {
		grid.rowUsed = row;
		grid.horizontalResizer.setTop(e.clientY - 475);
		grid.horizontalResizer.startResizing(e.clientY - 475);
